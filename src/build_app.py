...
rep = {'__GROUPS__': json.dumps(groups), '__STUDIES__': json.dumps([lab(s) for s in L['studies']]), '__ALPHAS__': json.dumps({lab(k): round(v, 4) for k, v in L['alphas'].items()}),
       ...
       '__OR_MM__': f"{C['OR_per_mmHg']:.2f}", '__NGROUPS__': str(L['groups']), '__REFS__': json.dumps(refs), '__CENTRED__': json.dumps(centred_pts), '__REFOPTS__': ''.join(f'<option value="{k}">{k} — {100*r["events"]/r["n"]:.0f}% overall</option>' for k, r in refs.items()), '__NSTUDIES__': str(len({s.split(' / ')[0] for s in L['studies']})),
