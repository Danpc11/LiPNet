...
refs = {k: v for k, v in refs.items() if '(SFSS)' in k}          # same outcome as the slope in use
rep = {'__GROUPS__': json.dumps(groups), '__STUDIES__': json.dumps([lab(s) for s in L['studies']]), '__ALPHAS__': json.dumps({lab(k): round(v, 4) for k, v in L['alphas'].items()}),
       ...
       '__OR_MM__': f"{B['OR_per_mmHg']:.2f}", '__REFS__': json.dumps(refs), '__ZGRID__': json.dumps(L['zgrid']), '__CENTRED__': json.dumps(centred_pts), '__REFOPTS__': ''.join(f'<option value="{k}">{k} — {100*r["events"]/r["n"]:.0f}% overall</option>' for k, r in refs.items()), '__NSTUDIES__': str(len(B['strata'])), '__NGROUPS__': str(B['groups']), '__EVENTS__': str(B['events']), '__PATIENTS__': str(B['patients']),}
