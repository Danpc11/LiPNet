...
B = json.load(open(f'{ROOT}/results/bayes_main.json'))['primary (SFSS or early dysfunction)']  # the paper's primary analysis
L = json.load(open(f'{ROOT}/results/within_study_fit.json'))                # only for the descriptive centred points
...
refs = {lab(g): dict(alpha=round(B['alphas'][g], 4), n=B['group_counts'][g]['n'], events=B['group_counts'][g]['events'],
                     zmin=B['group_counts'][g]['zmin'], zmax=B['group_counts'][g]['zmax']) for g in B['strata']}
centred_pts = [dict(s=lab(p['stratum']), zc=round(p['zP_c'], 4), lc=round(p['log_odds_c'], 4), n=p['n']) for p in C['points']]
for g in B['strata']: refs[lab(g)]['band'] = B['pred_band'][g]

rep = {'__GROUPS__': json.dumps(groups), '__STUDIES__': json.dumps([lab(s) for s in B['strata']]), '__ALPHAS__': json.dumps({lab(k): round(v, 4) for k, v in B['alphas'].items()}),
       ...
       '__OR_MM__': f"{B['OR_per_mmHg']:.2f}", '__REFS__': json.dumps(refs), '__ZGRID__': json.dumps(B['zgrid']), '__CENTRED__': json.dumps(centred_pts), '__REFOPTS__': ''.join(f'<option value="{k}">{k} — {100*r["events"]/r["n"]:.0f}% overall</option>' for k, r in refs.items()), '__NSTUDIES__': str(len(B['strata'])), '__NGROUPS__': str(B['groups']), '__EVENTS__': str(B['events']), '__PATIENTS__': str(B['patients']),}
