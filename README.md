...
**The effect of changing the gradient looks consistent.** Fitted with one intercept per stratum (study and outcome definition) and a common slope, on eleven groups from five series, 104 events across groups with a summed denominator of 734 recipients (Kyoto series overlap in time, so these are not necessarily 734 distinct people):

```
OR 7.09 per unit of zP (95% CI 3.37–20.5)   =   1.48 per mmHg of gradient
```

As a sensitivity analysis, a random-effects meta-analysis of the five per-series slopes agrees (OR 6.69, 95% CI 2.27–19.7 with a Hartung-Knapp interval, which is wide with five estimates) and detected no heterogeneity (τ² = 0, I² = 0%, Q = 1.47 on 4 df, p = 0.83), although with five estimates the power to detect it is low. The slope holds for both outcome definitions (1.87 and 2.03) and when any study is dropped (1.67–2.16). Centring each cohort on its own average puts all eleven groups on one scale (plot `centred`, weighted R² = 0.91); that view is descriptive, the effect comes from the stratified fit.

So the within-study estimates were consistently positive and compatible with a common pressure effect, while the level was not: that is the pattern prediction models usually show across sites, where the coefficients travel better than the intercept. Whether this particular effect transports has to be shown in data that were not used to fit it.
